...
public class KsjCaptureUtil {
    private static KsjCaptureUtil ksjCaptureUtil;
    cam ksjcam;
    Lock m_Lock;
    private String json;
    ...
    private KsjCaptureUtil() {
    }

/*    public static KsjCaptureUtil Instance(){
        if (ksjCaptureUtil == null){
            synchronized (KsjCaptureUtil.class){
                if (ksjCaptureUtil == null){
                    ksjCaptureUtil = new KsjCaptureUtil();
                }
            }
        }
        return ksjCaptureUtil;
    }*/
    ...
    private void changeSetting() {
        //bayer   BGGR      白平衡  HWB_PRESETTINGS
        currentBayer = KSJ_BAYERMODE.KSJ_RGGB_BGR32_FLIP.ordinal();
        int exTime = surfaceCamera.getExposureTime();
        if (exTime == 0){
            exTime = 200;
        }
        for(int i=0;i<ksjcam.m_devicecount;i++){
            ksjcam.SetBayerMode(i,currentBayer);
        ...
        surfaceCamera.setBayerMode(currentBayer);
    ...
    *//*String path = Environment.getExternalStorageDirectory().getAbsolutePath()+"/capture";
    ...
    private CaptureTaskUtil.OnCaptureFinishListener onCaptureFinishListener;
    public void setOnCaptureFinishListener(CaptureTaskUtil.OnCaptureFinishListener onCaptureFinishListener){
